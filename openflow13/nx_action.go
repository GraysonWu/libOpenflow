...
func (a *NXActionController2PropMaxLen) UnmarshalBinary(data []byte) error {
	...
	n := 0
	err := a.PropHeader.UnmarshalBinary(data[n:])
	if err != nil {
		return err
	}
	if len(data) < int(a.Length) {
		return errors.New("the []byte is too short to unmarshal a full NXActionController2Prop* message")
	}
	n += int(a.PropHeader.Len())
	a.MaxLen = binary.BigEndian.Uint16(data[n:])
	n += 2
...
func (a *NXActionController2) Len() uint16 {
	return a.NXActionHeader.Len() + 6 + uint16(len(a.props))*NXAC2_PROP_LEN
}
